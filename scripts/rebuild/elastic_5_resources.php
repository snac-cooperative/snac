...
if (\snac\Config::$USE_ELASTIC_SEARCH) {
    ...
    $db->cursorQuery("allResources","select rc.*, tv.value as document_type from
                    (select r.id, r.title, r.href, r.abstract, r.type from
                        ...
                        left join vocabulary tv on rc.type = tv.id");
    echo "Updating the Elastic Search indices. This may take a while...\n";
    $n = 0;
    while ($resource = $db->cursorFetch("allResources",$batchSize)) {
        if (pg_num_rows($resource) == 0) { break; }

        while($row = $db->fetchRow($resource)) {
            indexResource((int) $row["id"], $row["title"], $row["abstract"], $row["href"], $row["document_type"], (int) $row["type"]);
            $n++;
        }
        pg_free_result($resource);
        echo substr("$n    ",0,8);
    }
    $db->cursorClose("allResources");
    echo "\n";
...
function indexResource($id, $title, $abstract, $url, $type, $typeid) {
    global $eSearch, $primaryBody, $primaryCount, $batchSize;
    if ($eSearch != null) {
